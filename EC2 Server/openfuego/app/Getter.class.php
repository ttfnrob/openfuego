...
class Getter {
	...
	public function fuegoSize() {

		try {
			$dbh = $this->getDbh();
			$sql = "
				SELECT COUNT(*) as citizen_count FROM openfuego_citizens;
			";
		...
		if (!$items) {
			$citizen_count = 'Not available';
		} else {
			$citizen_count = $items[0]['citizen_count'];
		}

		try {
			$dbh = $this->getDbh();
			$sql = "
				SELECT COUNT(*) as link_count FROM openfuego_links;
			";
			$sth = $this->_dbh->prepare($sql);
			$sth->execute();
		} catch (\PDOException $e) {
...
*/
		...
		foreach ($items_filtered as $key => &$item_filtered) {
			...
			$tw_display_name = $status['display_name'];
			$tw_text = $status['text'];
			...
			$item_filtered['tw_display_name'] = $tw_display_name;
			// $item_filtered['tw_created_at'] = $tw_created_at;
