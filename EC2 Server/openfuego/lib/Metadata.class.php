...
class Metadata {
	...
	public function getTweet($link_id) {
		...
		try {
			$sql = "
				SELECT id_str, display_name, screen_name, text, profile_image_url
		...
		if ($tweets_cache_row) {  // if tweet CACHED
			...
			$display_name = $tweets_cache_row[1];
			$screen_name = $tweets_cache_row[2];
			$text = $tweets_cache_row[3];
			$profile_image_url = $tweets_cache_row[4];
			...
			if (empty($id_str) || $id_str == NULL) {
				...
				if (preg_match("/2../", $twitter->http_code)) {
					...
					try {
						$sql = "
							INSERT IGNORE INTO openfuego_tweets_cache (link_id, id_str, display_name, screen_name, text, profile_image_url)
							VALUES (:link_id, :id_str, :display_name, :screen_name, :text, :profile_image_url);
						";
						$sth = $dbh->prepare($sql);
						$sth->bindParam('link_id', $link_id);
		...
		$tweet = array('id_str' => $id_str, 'display_name' => $display_name, 'screen_name' => $screen_name, 'text' => $text, 'profile_image_url' => $profile_image_url);
	...
	public function updateTweet($link_id) {
		...
		$display_name = $search_result['user']['name'];
		...
		try {
			$sql = "
				INSERT INTO openfuego_tweets_cache
					(link_id, id_str, display_name, screen_name, text, profile_image_url)
				VALUES
					(:link_id, :id_str, :display_name, :screen_name, :text, :profile_image_url)
